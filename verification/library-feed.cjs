...
(async () => {
  ...
  const db = new Database('data/yemreact.sqlite');
  ...
  try {
    ...
    await go('/saved');
    assert.equal(await page.locator('#library-q, #sort, .duration-filter').count(), 3);
    ...
    report.checks.push('Preview, keyboard Escape, save actions and saved-page controls preserved');
